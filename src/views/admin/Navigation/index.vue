<template>
  <d2-container>
    <template slot="header">导航管理</template>
    <el-header>
    ...
    <el-table :data="data"
      ...
      <el-table-column prop="icon"
                       ...
                       align="center">
        <template slot-scope="scope">
          <i :class="['fa', 'fa-wa','fa-'+scope.row.icon]"></i>
        </template>
      </el-table-column>
        ...
        <el-form-item>
          <el-col :span="5">
            <el-form-item label="在新窗口打开"
                          :label-width="formLabelWidth">
              <el-switch v-model="form.isNewTab"
                         active-color="#13ce66">
              </el-switch>
            </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    // 加载数据
    loadData () {
      GetNavigation({ Sort: 'order' }).then(res => {
        this.data = res.list
      })
    },
    // 显示添加
    ...
    // 添加
    handleRowAdd () {
      AddNavigation(this.form).then(res => {
        console.log(res)
        this.$message({
          message: '保存成功',
          type: 'success'
    ...
    // 编辑
    handleRowEdit () {
      UpdataNavigation(this.form).then(res => {
        console.log(res)
        this.$message({
          message: '编辑成功',
          type: 'success'
